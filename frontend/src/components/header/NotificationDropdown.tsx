...
export default function NotificationDropdown() {
  ...
  const [items, setItems] = useState<Array<{id:number; title:string; body?:string|null; createdAt:string; readAt?:string|null}>>([]);
  ...
  const load = async () => {
    try {
      ...
      const mapped = data.map((n) => ({ id: Number(n.id), title: n.title, body: n.body || null, createdAt: n.createdAt, readAt: n.readAt || null }));
      setItems(mapped);
      setNotifying(mapped.some(n => !n.readAt));
    } catch (e) {
      // ignore
    } finally {
      setLoading(false);
    ...
    <div className="relative">
      ...
      <Dropdown isOpen={isOpen} onClose={closeDropdown} className="absolute -right-[240px] mt-[17px] flex h-[480px] w-[350px] flex-col rounded-2xl border border-gray-200 bg-white p-3 shadow-theme-lg dark:border-gray-800 dark:bg-gray-dark sm:w-[361px] lg:right-0">
        ...
        <ul className="flex flex-col h-auto overflow-y-auto custom-scrollbar">
          ...
          {!loading && items.length === 0 && <li className="px-4 py-3 text-sm text-gray-500">No notifications</li>}
          {!loading && items.map((n) => (
            <li key={n.id}>
              <DropdownItem
                onItemClick={async () => {
                  ...
                  try {
                    const token = localStorage.getItem('elog_token') || '';
                    await fetch(`${base}/notifications/${n.id}/read`, { method: 'PUT', headers: { Authorization: `Bearer ${token}` } });
                  } catch {}
                }}
              ...
              >
                <span className="block">
                  ...
                  <span className="flex items-center gap-2 text-gray-500 text-theme-xs dark:text-gray-400 mt-1">
                    ...
                    {!n.readAt && <span className="w-1.5 h-1.5 bg-orange-400 rounded-full"></span>}
                  </span>
                </span>
              </DropdownItem>
            </li>
